...
void CPopupSIOCtrlDlg::OnBnClickedButtonApply()
{
	if (m_pCameraHandle)
	{
		...
		stProperty.Parity = (ENeptuneSIOParity)m_cbParityBits.GetItemData(m_cbBaudRate.GetCurSel());
		stProperty.DataBit = (_uint32_t)m_cbDataBits.GetItemData(m_cbBaudRate.GetCurSel());
		stProperty.StopBit = (_uint32_t)m_cbStopBits.GetItemData(m_cbBaudRate.GetCurSel());
