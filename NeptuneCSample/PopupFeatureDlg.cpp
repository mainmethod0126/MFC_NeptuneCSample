		...
		for (int i = 0; i < MAX_ARRAY_COUNT; i++)
		{
			if (ntcGetFeature(m_pCameraHandle, m_arrMapping[i], &m_arrFeatureInfo[i]) == NEPTUNE_ERR_Success)
			{
				...
				if (m_arrFeatureInfo[i].Value != -1)
				{
					...
					m_sdValue[i].SetRange(m_arrFeatureInfo[i].Min, m_arrFeatureInfo[i].Max);
	...
	if (m_pCameraHandle)
	{
		...
		if (m_arrMapping[nIndex] == NEPTUNE_FEATURE_SNOWNOISEREMOVE)
		{
			...
			if (emErr == NEPTUNE_ERR_Success)
			{
				...
				m_sdValue[nIndex].EnableWindow(m_arrFeatureInfo[nIndex].bOnOff == NEPTUNE_BOOL_TRUE);
				m_edValue[nIndex].EnableWindow(m_arrFeatureInfo[nIndex].bOnOff == NEPTUNE_BOOL_TRUE);
			}
			else
			{
		...
		{
			stInfo.AutoMode = (m_ckAutoMode[nIndex].GetCheck() == BST_CHECKED) ? NEPTUNE_AUTO_CONTINUOUS : NEPTUNE_AUTO_OFF;
			ENeptuneError emErr = ntcSetFeature(m_pCameraHandle, m_arrMapping[nIndex], stInfo);
			if (emErr == NEPTUNE_ERR_Success)
			{
				memcpy(&m_arrFeatureInfo[nIndex], &stInfo, sizeof(NEPTUNE_FEATURE));
				BOOL bEnable = (m_arrFeatureInfo[nIndex].AutoMode == NEPTUNE_AUTO_OFF);
				m_sdValue[nIndex].EnableWindow(bEnable);
				m_edValue[nIndex].EnableWindow(bEnable);

				if (m_arrMapping[nIndex] == NEPTUNE_FEATURE_GAIN)
				{
					for (int i = 0; i < MAX_ARRAY_COUNT; i++)
					{
						if (m_arrMapping[i] == NEPTUNE_FEATURE_RGAIN ||
							...
							m_arrMapping[i] == NEPTUNE_FEATURE_BGAIN)
						{
							BOOL bIsAccess = (m_arrFeatureInfo[i].ValueAccessMode == NEPTUNE_NODE_ACCESSMODE_RW || m_arrFeatureInfo[i].ValueAccessMode == NEPTUNE_NODE_ACCESSMODE_WO);
							m_sdValue[i].EnableWindow(bEnable && bIsAccess);
							m_edValue[i].EnableWindow(bEnable && bIsAccess);
						}
						else if (m_arrMapping[i] == NEPTUNE_FEATURE_AUTOGAIN_MIN ||
								 m_arrMapping[i] == NEPTUNE_FEATURE_AUTOGAIN_MAX)
						{
							m_sdValue[i].EnableWindow(!bEnable);
							m_edValue[i].EnableWindow(!bEnable);
						}
					}
				}
				else if (m_arrMapping[nIndex] == NEPTUNE_FEATURE_SHUTTER)
				{
					for (int i = 0; i < MAX_ARRAY_COUNT; i++)
					{
						if (m_arrMapping[i] == NEPTUNE_FEATURE_AUTOSHUTTER_MIN ||
							m_arrMapping[i] == NEPTUNE_FEATURE_AUTOSHUTTER_MAX ||
							m_arrMapping[i] == NEPTUNE_FEATURE_AUTOEXPOSURE)
						{
							m_sdValue[i].EnableWindow(!bEnable);
							m_edValue[i].EnableWindow(!bEnable);
						}
